...
K = 20
MAX_SAMPLES = 10_000    # Only compute metrics over up to 10k samples for efficiency
...
def load_prediction_baseline(baseline_type, n_latents, nu, path=PATHS[SUBPROJECT]):
    if baseline_type == 'dp':
        prediction_model = joblib.load(
            os.path.join(path, 'dp', f'dp_pca_ssh_d{n_latents}_nu{nu}.joblib')
        )
    elif baseline_type == 'lim':
        prediction_model = joblib.load(
            os.path.join(path, 'lim', f'lim_pca_ssh_d{n_latents}_nu{nu}.joblib')
        )
...
def main():
    ...
    utils.log(f"Load PCA model for D={N_LATENTS}", START_TIME)
    pca_model = joblib.load(
        os.path.join(PATHS[SUBPROJECT], 'pca', f'pca_ssh_{N_LATENTS}.joblib')
    )
    ...
    def process(nu):
        ...
        models = (pca_model, prediction_model)
        mse = compute_mse(models, X_val, mask, weights, n_iter=K)
        return mse.mean(dim='variable').values

    mse_all = PARALLEL(
        joblib.delayed(process)(nu) for nu in tqdm(np.arange(1, K+1))
    )
    mse_all = np.vstack(mse_all)
    ...
    mse_all.to_netcdf(
        os.path.join(output_path, f'pca_{BASELINE}_ssh_d{N_LATENTS}_val_mse.nc')
    )
    ...
    best_baseline = joblib.load(
        os.path.join(
            PATHS[SUBPROJECT],
            BASELINE,
            f'{BASELINE}_pca_ssh_d{N_LATENTS}_nu{optimal_nu}.joblib'
        )
    )
    ...
    joblib.dump(
        ...
        os.path.join(
            PATHS[SUBPROJECT],
            BASELINE,
            f'{BASELINE}_pca_ssh_d{N_LATENTS}_best.joblib'
        )
    )

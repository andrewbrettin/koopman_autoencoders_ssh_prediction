...
Fits Damped persistence model to data from the CNN experiments
"""
...
NU_MAX = 10 if SAMPLING == 'monthly' else 20  # Maximum number of reccurent passes for Koopman
...
def main():
    ...
    for nu in np.arange(1, NU_MAX+1):
        ...
        print(y_train_weighted)
        
        utils.log(f"Training DP with D={N_LATENTS}", START_TIME)
        # Load PCA Model
        ...
        dp_model = linear_models.DampedPersistenceModel()
        dp_model.fit(X=z_train, y=z_train_next, nu=nu)

        # Save damped persistence model
        utils.log(f"Saving DP model to {PATHS[subproject_name]}", START_TIME)
        path = os.path.join(PATHS[subproject_name], 'dp')
        os.makedirs(path, exist_ok=True)
        filename = f'dp_pca_ssh_d{N_LATENTS}_nu{nu}.joblib'
        joblib.dump(dp_model, os.path.join(path, filename))
